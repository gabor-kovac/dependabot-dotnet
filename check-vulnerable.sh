...
if jq -e '.. | objects | select(has("vulnerabilities"))' vulnerable_packages.json > /dev/null 2>&1; then
    echo "Vulnerable packages detected:" >&2
    jq '.. | objects | select(has("vulnerabilities")) | .vulnerabilities' vulnerable_packages.json >&2
    exit 1
else
